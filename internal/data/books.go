...
func (m BookModel) Insert(book *Book) error {
	...
	return m.Pool.QueryRow(ctx, query, args...).Scan(&book.ID, &book.CreatedAt, &book.Version)
}
...
func (m BookModel) Update(book *Book) error {
	query := `
		UPDATE movies
		SET title = $1, year = $2, genres = $3, version = uuid_generate_v4()
	...
	err := m.Pool.QueryRow(ctx, query, args...).Scan(&book.Version)
	if err != nil {
		switch {
		case errors.Is(err, pgx.ErrNoRows):
